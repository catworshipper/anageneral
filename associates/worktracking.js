...
async function sendClockInEmail() {
  ...
  await fetch(`${SUPABASE_URL}/functions/v1/send-email`, {
    ...
    body: JSON.stringify({
      ...
      to: [authState.appUser.email, 'automation@topwebweb.com'].filter(Boolean),
      data: emailData,
    }),
  });
}
...
async function handleRequestPayment() {
  ...
  try {
    ...
    body += `</table>`;
    if (hasRateMismatch) {
      body += `<p style="color:#92400e;"><strong>Action needed:</strong> Some entries were recorded at the wrong rate. Go to <a href="https://topwebweb.com/spaces/admin/worktracking.html">Admin Hours</a>, filter by ${escapeHtml(name)}, select all unpaid entries, and click "Recalc" to update them to the current rate. Then "Mark Selected as Paid" to process payment.</p>`;
    } else {
      body += `<p>Go to <a href="https://topwebweb.com/spaces/admin/worktracking.html">Admin Hours</a> to review and process payment.</p>`;
    }
